'''
This file contains all quantum algorithms used in QMES
'''
...
def QFT(nq):
    '''
    Basic circuit for a quantum Fourier transform of nq qubits
    
    Parameter: 
        nq, the number of qubits
    
    Return:
        a composite gate of the QFT circuit of nq qubits
    '''
...
def QPE(U, nq_phase, nq_vec):
    ''' 
    quantum circuit for quantum phase estimation (Fig. 1 on p. 3)
    
    Parameters:
        U (QuantumCircuit) - the unitary operator????
        nq_phase (int), the number of qubits used to store the phase
        nq_vec (int), the number of qubits to represent an eigenvector of U 
        
    Returns: 
        the gate encapsulating the QPE circuit
    '''
